  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if ($user_id) {
        if ($post_id && $ontology && $reply_content) {
          ...
          $results = mysqli_query($db, sprintf("INSERT INTO `tbl_post_reply` (user_id, post_id,ontology,reply_content,confidence_score,flag,time_stamp) VALUES ('%s', '%s', '%s', '%s', '%s', '%s','%s')", $user_id, $post_id, $ontology, $reply_content,$confidence_score,'0',$time_stamp));
